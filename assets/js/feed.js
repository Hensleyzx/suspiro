(async function(){
  ...
  function renderComments(p){
    ...
    root.innerHTML=p.comments.length?p.comments.map(c=>{
      ...
      return `<div class="comment">${profileAvatar(prof,'sm')}<div><strong>${esc(prof.display_name||prof.username)}</strong><span>@${esc(prof.username)}</span><p>${esc(c.body)}</p></div></div>`;
  ...
  async function renderSuggestions(){
    ...
    try{
      ...
      root.innerHTML=(people||[]).map(p=>{
        const status=followMap.get(p.id);return `<div class="suggestion">${profileAvatar(p,'sm')}<div class="meta"><strong>@${esc(p.username)}</strong><span>${p.is_private?'Perfil privado':'Perfil público'}</span></div><button class="btn ${status?'btn-primary':'btn-outline'} btn-sm" data-suggest-id="${p.id}">${status==='pending'?'Solicitado':status?'Seguindo':'Seguir'}</button></div>`;
