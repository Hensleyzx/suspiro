(async function(){
  ...
  function renderPeople(){
    ...
    row.innerHTML=filtered.map(p=>{
      ...
      return `<div class="person-card" data-person-card>${avatarHtml(p)}<strong>@${S.esc(p.username)}</strong><span class="compat">${score}% em comum</span><span>${S.esc(favs|| (p.is_private?'Perfil privado':'Novo no Suspiro'))}</span><button class="btn ${status?'btn-primary':'btn-outline'} btn-sm" data-follow-id="${p.id}">${status==='pending'?'Solicitado':status?'Seguindo':'Seguir'}</button><a class="btn btn-ghost btn-sm" href="messages.html?user=${encodeURIComponent(p.id)}">Mensagem</a></div>`;
    }).join('');
    ...
    document.getElementById('best-match-name').textContent=best?`@${best.username}`:'Sem resultado';
    renderPopular();
  }
